...
namespace superpunto{
  RRenderer::RRenderer(std::shared_ptr<System> sys, std::shared_ptr<RWindow> in_w, std::shared_ptr<Camera> cam, float gscale):
    sys(sys),
    cam(cam),
    gscale(gscale),
    box(gscale, &MVP),
    axis(sys, in_w, &MVP, glm::vec3(-10,0,0)),
    textRenderer(sys, in_w),
    w(in_w){
    picked[0] = picked[1] = -1;
    textRenderer.setFont(sys->getInputOptions().fontName.c_str(), 45);
    auto resolution = w->getResolution();
    auto op = sys->getInputOptions();
    proj = glm::perspective(op.fov, resolution.x/(float)resolution.y, znear, zfar);
    model = glm::mat4();    
    view = cam->lookAt();
  ...
  void RRenderer::display(){
    ...
    if(!sys->getInputOptions().noaxis) axis.draw(cam->pos);
  ...
  Uint8* RRenderer::getPixels(){
    ...
    glReadPixels(0,0, resolution.x,resolution.y, GL_RGBA, GL_UNSIGNED_BYTE, (void *)cdata.data());
    return cdata.data();
  }
...
#include"shaders.h"
  ...
  RAxis::RAxis(std::shared_ptr<System> sys, std::shared_ptr<RWindow> w, glm::mat4 *MVP, glm::vec3 origin):
    MVP(MVP),
    axislength(5.0f),
    origin(origin),
    sys(sys),
  ...
  void RAxis::draw(glm::vec3 campos){
    ...
    auto resolution = w->getResolution();
    Xtext.setText("X", 0.5f*(xpos.x+1)*resolution.x, resolution.y-0.5f*(xpos.y+1)*resolution.y, 35.f/dist);
    Ytext.setText("Y", 0.5f*(ypos.x+1)*resolution.x, resolution.y-0.5f*(ypos.y+1)*resolution.y, 35.f/dist);
    Ztext.setText("Z", 0.5f*(zpos.x+1)*resolution.x, resolution.y-0.5f*(zpos.y+1)*resolution.y, 35.f/dist);
