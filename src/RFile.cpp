...
namespace superpunto{
  ...
  void RFile::setFrame(int frame){
	...
	for(int i = storedFrames; i<frame+1; i++)
	  if(!readNextFrame()) break;
      }
      else{
	if(!end_of_file)sys->log<System::DEBUG1>("[File] Busy Waiting for frame %d, current %d", frame, (int)storedFrames);
	while(frame>=storedFrames and !end_of_file){}
      }
    }
    
    if(frame>=storedFrames){
      sys->log<System::DEBUG>("[File] Could not read frame %d, only %d frames in file %s",
			      frame, (int)storedFrames, sys->getInputOptions().readFile.c_str());
    }
  ...
  void RFile::setNextFrame(){
    setFrame(current_frame+1);
  }
  void RFile::setPreviousFrame(){
    setFrame(current_frame-1);
  }
  ...
  bool RFile::readNextFrame(){
    ...
    if(numberChars <=0 or !in.good()){
      sys->log<System::DEBUG>("[File] End of file reached, total frames read: %d", (int)storedFrames);
      end_of_file = true;
      return false;
    }
    ...
    while(!in.eof() and in.good()){
      numberChars = in.getNextLine(line);
      if(numberChars<=0 or in.iscomment(line, numberChars, '#')) break;
      //Count number of columns from first row
      if(numberParticles == 0){
	std::stringstream is(line);
	float number;
	while(is>>number) numberColumns++;
	if(numberColumns==0) break;
    ...
    if(in.eof() or !in.good() or numberChars <= 0){
      sys->log<System::DEBUG>("[File] End of file reached, total frames read: %d", storedFrames+1);
      end_of_file = true;
    }

    if(numberParticles<1){
      sys->log<System::DEBUG>("[File] Empty frame detected: %d", (int)storedFrames);
    ...
    sys->log<System::DEBUG1>("[File] Frame %d read with %d particles, %d columns per particle",
			    (int)storedFrames, numberParticles, numberColumns);
    Natframe.push_back(numberParticles);
    current_pos.shrink_to_fit(); pos.emplace_back(std::move(current_pos));
    if(current_vel.size()){
      current_vel.shrink_to_fit(); vel.emplace_back(std::move(current_vel));
    }
    current_scales.shrink_to_fit(); scales.emplace_back(std::move(current_scales));
    current_colors.shrink_to_fit(); colors.emplace_back(std::move(current_colors));    
    storedFrames++;
  ...
  //Takes the number of columns read in a row and reorganizes them such that for a given particle:
  //
  //  row[0,1,2] = pos
  //  row[3] = radius;  
  //  row[4] = color;
  ...
  void RFile::parseRowNumbers(float *row_numbers, int numberColumns){
    if(numberColumns<4){
      if(numberColumns==1) row_numbers[1] = 0.0;
      if(numberColumns==2) row_numbers[2] = 0.0;
      row_numbers[3] = 0.5;
      row_numbers[4] = 0;
    }
    if(numberColumns==4){
