...
void System::parseArguments() {
  ...
  fori(0, m_argc) {
    ...
    if (strcmp(m_argv[i], "--RGB") == 0)
      options.read_color_mode = ColorParserType::HEXBGR; // readHexColor = true;
    ...
    if (strcmp(m_argv[i], "--binary") == 0)
      options.binary_read_mode = true;
    if (strcmp(m_argv[i], "--noaxis") == 0)
      options.noaxis = true;
    if (strcmp(m_argv[i], "--nobox") == 0)
      options.nobox = true;
    if (strcmp(m_argv[i], "--use-font") == 0)
      options.fontName = std::string(m_argv[i + 1]);
  }
  ...
  if (isatty(STDIN_FILENO)) { // If there is no pipe
    ...
    fori(1, m_argc) {
      auto in = std::ifstream(
          m_argv[i]); // There must be a filename somewhere in the cli
      if (in.good()) {
        good_file = true;
...
void System::printHelp() {
  ...
  log<System::MESSAGE>("\t  --record :  Makes a movie of all the frames in "
                       "file and generates a .mp4");
  log<System::MESSAGE>(
      "\t  --frames-between-screenshots X : Number of frames skipped between "
  ...
  log<System::MESSAGE>("\t  --resolution X Y : Set the resolution (AKA window "
                       "size). Default is 800x800");
  ...
  log<System::MESSAGE>("\t Go back to the initial view with 0");
  ...
  log<System::MESSAGE>("\t Press C to take a screenshot in png\t Press L to "
                       "play and record to a mp4 until L is pressed again");
  log<System::MESSAGE>("  Others:");
  log<System::MESSAGE>("\t Press h to print this help page");
}

void System::printWelcome() {
  log<System::MESSAGE>("Welcome to Superpunto. ver. %d.%d! Press h for help",
                       SUPERPUNTO_MAJOR, SUPERPUNTO_MINOR);
}
