...
namespace superpunto{
  ...
  RTextRenderer::RTextRenderer(std::shared_ptr<System> sys, std::shared_ptr<RWindow> w):
    sys(sys),
    size(glm::vec2(0)),
    pos(glm::vec2(0)),
    size_factor(1.0f),
...
#include"shaders.h"
  ...
  bool RTextRenderer::setText(const char*text, int x, int y, float sf){
    ...
    this->size = glm::vec2(surf->w/(float)w->getResolution().x, surf->h/(float)w->getResolution().y);
    pr->use();
  ...
  void RTextRenderer::draw(){
    ...
    glUniform2f(glGetUniformLocation(pr->id(), "size"), size.x, size.y);
    glUniform2f(glGetUniformLocation(pr->id(), "pos"), pos.x, pos.y);
    glUniform1f(glGetUniformLocation(pr->id(), "FWIDTH"), (float)w->getResolution().x);
    glUniform1f(glGetUniformLocation(pr->id(), "FHEIGHT"), (float)w->getResolution().y);
